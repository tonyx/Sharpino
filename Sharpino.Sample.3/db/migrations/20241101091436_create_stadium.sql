...
CREATE TABLE public.aggregate_events_01_stadium (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_stadium_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
...
create index ix_01_events_stadium_id on public.events_01_stadium(aggregate_id);
create index ix_01_aggregate_events_stadium_id on public.aggregate_events_01_stadium(aggregate_id);
create index ix_01_snapshot_stadium_id on public.snapshots_01_stadium(aggregate_id);
create index ix_01_snapshot_stadium_aggregate_id_and_id on public.snapshots_01_stadium(aggregate_id, id DESC);
create index ix_01_snapshot_stadium_event_id on public.snapshots_01_stadium(event_id);
create index ix_01_events_stadium_timestamp on public.events_01_stadium("timestamp");
create index ix_01_snapshots_stadium_timestamp on public.snapshots_01_stadium("timestamp");
...
CREATE OR REPLACE FUNCTION insert_md_01_stadium_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
...
inserted_id integer;
BEGIN
INSERT INTO events_01_stadium(event, aggregate_id, distance_from_latest_snapshot, timestamp, md)
VALUES(event_in::text, aggregate_id, distance_from_latest_snapshot, now(), md) RETURNING id INTO inserted_id;
return inserted_id;
END;
$$;

CREATE OR REPLACE FUNCTION insert_01_stadium_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_stadium_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text   
