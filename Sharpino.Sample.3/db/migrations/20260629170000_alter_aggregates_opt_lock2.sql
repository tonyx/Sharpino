...
CREATE OR REPLACE FUNCTION insert_md_01_seatrow_aggregate_event_and_return_id_opt_lock2(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_stadium_aggregate_event_and_return_id_opt_lock2(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text,   
...
DROP FUNCTION IF EXISTS insert_md_01_seatrow_aggregate_event_and_return_id_opt_lock2(
    text, uuid, int, text, int, text[], int[], uuid[]
);

DROP FUNCTION IF EXISTS insert_md_01_stadium_aggregate_event_and_return_id_opt_lock2(
    text, uuid, int, text, int, text[], int[], uuid[]
);
