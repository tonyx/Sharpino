...
type TodoManager(messageSenders: MessageSenders, eventStore: IEventStore<string>, todosViewer: AggregateViewer<Todo>) =
    new() =
        ...
        let viewer: AggregateViewer<Todo> =
            fun id -> getAggregateStorageFreshStateViewer<Todo, TodoEvents, string> memoryStorage id

        TodoManager(NoSender, memoryStorage, viewer)

    member this.AddTodo(todo: Todo) =
        result { return! todo |> runInit<Todo, TodoEvents, string> eventStore messageSenders }
    ...
    member this.GetTodosAsync(?ct: CancellationToken) =
        taskResult {
            ...
            let! todos =
                StateView.getAggregateStatesInATimeIntervalAsync<Todo, TodoEvents, string>
                    eventStore
                    DateTime.MinValue
                    DateTime.MaxValue
                    (ct |> Some)
    ...
    member this.ReplaceSensibleDataEvents (id: TodoId) (dummyData: string) =
        taskResult {
            ...
            let! replaceSensibleData =
                eventStore.GDPRReplaceEventsByPredicate
                    Todo.Version
                    Todo.StorageName
                    id.Value
                    composedReplacementPredicate
                    serializedUpdatePrivateData

            return ()
        }

    member this.ReplaceSensibleDataEventsAsync (id: TodoId) (dummyData: string) =
        taskResult {
            let replacement = TodoEvents.PrivateDataUpdated dummyData
            let serializedUpdatePrivateData = replacement.Serialize
            let replacementPredicate = fun (x: TodoEvents) -> x.IsPrivateDataUpdated
            ...
            let! replaceSensibleData =
                eventStore.GDPRReplaceEventsByPredicateAsync(
                    Todo.Version,
                    Todo.StorageName,
                    id.Value,
                    composedReplacementPredicate,
