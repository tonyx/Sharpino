...
let marketInstances =
    [
          Supermarket(eventStore, messageSender, goodsContainerViewer, goodsViewer, cartViewer ), "eventStorePostgres", setupPgEventStore, goodsViewer, eventStore:> IEventStore<byte[]>  ;
         // Supermarket(eventStore, messageSender, goodsContainerViewer, goodsViewer, cartViewer ), "eventStorePostgres", setupPgEventStore, goodsViewer  ;
    ]
...
let tests =
    ...
    testList "samples" [
        ...
        // will need using delete
        multipleTestCase  "when remove a good then can't gets its quantity - Error" marketInstances <| fun (supermarket, _, setup, _, _) ->
            setup ()
            let good = Good.MkGood (Guid.NewGuid(), "Good", 10.0m)
            let added = supermarket.AddGood good
            Expect.isOk added "should be ok"
