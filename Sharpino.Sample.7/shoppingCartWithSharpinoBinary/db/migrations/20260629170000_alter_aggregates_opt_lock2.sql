...
CREATE OR REPLACE FUNCTION insert_md_01_cart_aggregate_event_and_return_id_opt_lock2(
    IN event_in bytea,
...
CREATE OR REPLACE FUNCTION insert_md_01_good_aggregate_event_and_return_id_opt_lock2(
    IN event_in bytea,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text,   
...
DROP FUNCTION IF EXISTS insert_md_01_cart_aggregate_event_and_return_id_opt_lock2(
    bytea, uuid, int, text, int, text[], int[], uuid[]
);

DROP FUNCTION IF EXISTS insert_md_01_good_aggregate_event_and_return_id_opt_lock2(
    bytea, uuid, int, text, int, text[], int[], uuid[]
);
