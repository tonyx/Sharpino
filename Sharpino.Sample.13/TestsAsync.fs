module Sharpino.Sample._13.TestsAsync
open DotNetEnv
open System
...
let pgStorageUsersViewerAsync = fun id -> getAggregateStorageFreshStateViewerAsync<User, UserEvent, string> pgEventStore None id |> Async.AwaitTask |> Async.RunSynchronously
let pgStorageReservationsViewerAsync = fun id -> getAggregateStorageFreshStateViewerAsync<ReservationForNickNames, ReservationEvent, string> pgEventStore None id |> Async.AwaitTask |> Async.RunSynchronously

// in an async setup the reservation pattern may have unpredictable results 
[<Tests>]
let tests =
    ptestList "samples async" [
        testCase "register a single user - Ok" <| fun _ ->
            ...
            let registrationManager =
                UsersRegistrationManager
                    (
                        pgEventStore,
                        pgStorageUsersViewerAsync,
                        pgStorageReservationsViewerAsync,
