...
CREATE OR REPLACE FUNCTION insert_md_01_site_aggregate_event_and_return_id_opt_lock(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_truck_aggregate_event_and_return_id_opt_lock(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text,   
...
DROP FUNCTION IF EXISTS insert_md_01_site_aggregate_event_and_return_id_opt_lock(
    text, uuid, int, text, int
);

DROP FUNCTION IF EXISTS insert_md_01_truck_aggregate_event_and_return_id_opt_lock(
    text, uuid, int, text, int
);
