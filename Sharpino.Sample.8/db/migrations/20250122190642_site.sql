...
CREATE TABLE public.aggregate_events_01_site (
                                                 id integer DEFAULT nextval('public.aggregate_events_01_site_id_seq') NOT NULL,
                                                 aggregate_id uuid NOT NULL,
                                                 event_id integer
);
...
CREATE OR REPLACE FUNCTION insert_01_site_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_site_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_site_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_site_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
...
CREATE OR REPLACE FUNCTION insert_enhanced_01_site_aggregate_event_and_return_id(
       IN event_in text,
