...
open Sharpino.Sample.Saga.Domain.Booking.Consumer
open Sharpino.Sample.Saga.Domain.Booking.Consumer.BookingConsumer
open Sharpino.Sample.Saga.Domain.Rows.Consumer.RowsConsumer
open Sharpino.Sample.Saga.Domain.Voucher.Consumer.VoucherConsumer
...
let bookingConsumer =
    ...
    |> Seq.find (fun s -> s.GetType() = typeof<BookingConsumer>)
    :?> BookingConsumer

let voucherConsumer =
    host.Services.GetServices<IHostedService>()
    |> Seq.find (fun s -> s.GetType() = typeof<VoucherConsumer>)
    :?> VoucherConsumer

let rowConsumer =
    host.Services.GetServices<IHostedService>()
    |> Seq.find (fun s -> s.GetType() = typeof<RowsConsumer>)
    :?> RowsConsumer

let rabbitMqBookingStateViewer = bookingConsumer.GetAggregateState
let rabbitMqVoucherStateViewer = voucherConsumer.GetAggregateState
let rabbitMqRowStateViewer = rowConsumer.GetAggregateState

let aggregateMessageSenders = System.Collections.Generic.Dictionary<string, MessageSender>()

let seatsMessageSender = mkMessageSender "127.0.0.1" (Row.Version + Row.StorageName) |> Result.get
let bookingsMessageSender = mkMessageSender "127.0.0.1" (Booking.Version + Booking.StorageName) |> Result.get
let vouchersMessageSender = mkMessageSender "127.0.0.1" (Voucher.Version + Voucher.StorageName) |> Result.get

aggregateMessageSenders.Add (Row.Version + Row.StorageName, seatsMessageSender)
aggregateMessageSenders.Add (Booking.Version + Booking.StorageName, bookingsMessageSender)
...
let bookingsAggregateMemoryViewer: Guid -> Result<(EventId * Booking), string> =
    fun id ->
        result
            {
...
let vouchersAggregateMemoryViewer =
    fun id ->
        result
            {
...
let theaterContextdbViewer = getStorageFreshStateViewer<Theater, TheaterEvents, string> dbEventStore 
...
let tests =
    testList "samples" [
        multipleTestCase "fresh seat has zero rows (with parameters) - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
        ...
        multipleTestCase "fresh seat has zero rows - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "seat service has zero rows - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
        ...
        multipleTestCase "seat service has zero booking - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
        ...
        multipleTestCase "add and retrieve a row - Ok" appVersionsEnvs <| fun (setup, dbinfo, service, delay) ->
            setup ()
            let service = service ()
            let id = Guid.NewGuid()
        ...
        multipleTestCase "add and retrieve a booking - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
        ...
        multipleTestCase "assign a booking to a row and verify that the booking has a rowId set to that rowId - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
            let row = mkDefaultRow20Seats()
            let addRow = service.AddRow row 
        ...
        multipleTestCase "assign a booking to a row and verify that the row has the bookingId in the AssociatedBookings list - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
            let row = mkDefaultRow20Seats()
            let addRow = service.AddRow row 
        ...
        multipleTestCase "assign a booking where the number of claimed seats is superior than the availability - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
        ...
        multipleTestCase "in doing two consecutive bookings that succeeds, the number of free seats is the initial minus the sum of the claimed seats - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
            let row = mkDefaultRow10Seats()
            let addRow = service.AddRow row 
        ...
        multipleTestCase "make two consecutive bookings, the second one exceeds the total and therefore it fails whereas the number of remaining seats is the initial minus the number related to the first booking - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service ()
            let row = mkDefaultRow10Seats()
        ...
        multipleTestCase "can do in parallel two bookings on two different seats - OK" appVersionsEnvs  <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service ()
        ...
        multipleTestCase "do parallel bookings on two different seats whereas one of the booking can't succeed, so all the bookings must fail - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service() 
            let row1 = mkDefaultRow10Seats()
        ...
        multipleTestCase "can't do in parallel two bookings on the same row - Error" appVersionsEnvs  <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service ()
            let row = mkDefaultRow10Seats()
        ...
        multipleTestCase "do in parallel two bookings on two different seats using no different id checks, works as in the normal case - OK" appVersionsEnvs  <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service ()
            let row1 = mkDefaultRow10Seats()
        ...
        multipleTestCase "do in parallel two bookings on the same row using no id unique check so the result is ok and the resulting state is not correct because globally the result is not valid  - Error" appVersionsEnvs  <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service () 
            let row = mkDefaultRow10Seats()
        ...
        multipleTestCase "do in sequence two bookings on the same row using saga so the resulting state is correct - OK" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "do in sequence two bookings on the same row using saga so the resulting state is correct, use prevalidation - OK" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "do in sequence using saga way a transaction that will exceeds the available seats and so it will rollback - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service () 
            let row = mkDefaultRow10Seats()
        ...
        multipleTestCase "can't assign booking, not enough seats - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            // preparation
            let service = service () 
            let row = mkDefaultRow10Seats()
        ...
        multipleTestCase "a more generalized saga example - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
            // preparation
            let row = mkDefaultRow20Seats()
        ...
        multipleTestCase "a more generalized non example using repeatedly same aggregate IDs - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let service = service ()
            // preparation
            let row = mkDefaultRow20Seats()
        ...
        multipleTestCase "a more generalized saga example where compensation take place - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "a more generalized no saga example - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "add a row and then new seats to that row - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "add a row and then remove seats from that row - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "add a row, then add a booking, then remove some of the seats that are left free - Ok" appVersionsEnvs  <| fun (setup, _, service, delay) ->
            setup()
            let seatBookingService = service ()
        ...
        multipleTestCase "add a row, then add a booking, try to remove more seats that available - Error" appVersionsEnvs  <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "remove zero seats using saga like multicommand - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "remove a seat from a row - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "remove three seats using saga like multicommand, two different removals, - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "remove three seats using prevalidation, two different removals, - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, one shot - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, two shots - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, prevalidation  - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, use 'force' version - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, two shots, use 'force' version - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, three shots, case 1 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, three shots - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "trying to remove more seats than existing ones, three shots (use 'force') - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "a more generalized saga example where compensation take place 2 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
        ...
        multipleTestCase "a more generalized example using 'force' instead of saga - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()    
            let seatBookingService = service ()
            let row = mkDefaultRow20Seats()
        ...
        multipleTestCase "a more generalized saga example of compensation 3 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "a more generalized example using 'force' instead of saga 3 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "a more generalized saga example of compensation 4 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "a more generalized example using 'force' instead of saga 4 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup()
            let seatBookingService = service ()
        ...
        multipleTestCase "a more general saga example of compensation 5 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            // preparation
            setup()
            let seatBookingService = service ()
        ...
        multipleTestCase "a more general example using 'force' instead of saga 5 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            // preparation
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "a more general saga example of compensation 6 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
        ...
        multipleTestCase "a more general example of using 'force' instead of saga 6 - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup()
        ...
        multipleTestCase "add and retrieve a voucher - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "add a voucher, a row and a boooking, will be able to spend the voucher - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup()
            let seatBookingService = service ()
            let voucher = { Id = Guid.NewGuid(); Capacity = 10}
            let addVoucher = seatBookingService.AddVoucher voucher
        ...
        multipleTestCase "add a voucher, a row and a booking, seats are more than claimed booking - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup()
            let seatBookingService = service ()
            let voucher = { Id = Guid.NewGuid(); Capacity = 10}
            let addVoucher = seatBookingService.AddVoucher voucher
        ...
        multipleTestCase "add a voucher, a row and a booking, voucher capacity is less than seats in the row - Error" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "add two vouchers, two rows and two booking, will be able to assign bookings and voucher as the voucher capacity and claimed seats are less than the seats in the row in each case - Ok " appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup()
            let seatBookingService = service ()
        ...
        multipleTestCase "add two vouchers, two rows and two booking, will be able to assign bookings and voucher as the voucher capacity and claimed seats are less than the seats in the row in each case, use forceAssign - Ok " appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
            let voucher1 = { Id = Guid.NewGuid (); Capacity = 10}
            let voucher2 = { Id = Guid.NewGuid (); Capacity = 10}
        ...
        multipleTestCase "add two vouchers, two rows and two booking. Will be able to assign bookings and voucher as the voucher capacity and claimed seats are less than the seats in the row in each case, use forceAssign, some voucher are not left empty - Ok " appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "reuse the same voucher more than once provided that it has enough capacity  - Ok " appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "the voucher capacity should be able to fulfill only the first row, so we expect an Error " appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
            let voucher = { Id = Guid.NewGuid (); Capacity = 10}
        ...
        multipleTestCase "the voucher capacity should be able to fulfill only the first two rows, so we expect an Error " appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
        ...
        multipleTestCase "one voucher sufficient enough, three rows and three boookings enough for each rows - Ok" appVersionsEnvs <| fun (setup, _, service, delay) ->
            setup ()
            let seatBookingService = service ()
