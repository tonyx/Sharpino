...
CREATE TABLE public.aggregate_events_01_teacher (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_teacher_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
                                                    event_id integer
);
...
create index ix_01_events_teacher_timestamp on public.events_01_teacher("timestamp");
create index ix_01_snapshots_teacher_timestamp on public.snapshots_01_teacher("timestamp");

CREATE OR REPLACE FUNCTION insert_01_teacher_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_teacher_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_teacher_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_teacher_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
