...
CREATE TABLE public.aggregate_events_01_row (
                                                id integer DEFAULT nextval('public.aggregate_events_01_row_id_seq') NOT NULL,
                                                aggregate_id uuid NOT NULL,
                                                event_id integer
);
...
CREATE OR REPLACE FUNCTION insert_01_row_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_row_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_row_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_row_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
...
CREATE OR REPLACE FUNCTION insert_enhanced_01_row_aggregate_event_and_return_id(
       IN event_in text,
...
event_id integer;
    max_id integer;
BEGIN
SELECT COALESCE(MAX(id), 0) INTO max_id FROM events_01_row WHERE aggregate_id = p_aggregate_id;
