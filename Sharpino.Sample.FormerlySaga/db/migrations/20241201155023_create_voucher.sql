...
CREATE TABLE public.aggregate_events_01_voucher (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_voucher_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
                                                    event_id integer
);
...
CREATE OR REPLACE FUNCTION insert_01_voucher_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_01_voucher_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
