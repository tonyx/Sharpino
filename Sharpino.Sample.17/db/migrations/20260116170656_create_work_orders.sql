...
CREATE TABLE public.aggregate_events_01_WorkOrders (
                                                      id integer DEFAULT nextval('public.aggregate_events_01_WorkOrders_id_seq') NOT NULL,
                                                      aggregate_id uuid NOT NULL,
...
create index ix_01_events_WorkOrders_id on public.events_01_WorkOrders(aggregate_id);
create index ix_01_aggregate_events_WorkOrders_id on public.aggregate_events_01_WorkOrders(aggregate_id);
create index ix_01_snapshot_WorkOrders_id on public.snapshots_01_WorkOrders(aggregate_id);
create index ix_01_snapshot_WorkOrders_event_id on public.snapshots_01_WorkOrders(event_id);
create index ix_01_events_WorkOrders_timestamp on public.events_01_WorkOrders("timestamp");
create index ix_01_snapshots_WorkOrders_timestamp on public.snapshots_01_WorkOrders("timestamp");

CREATE OR REPLACE FUNCTION insert_01_WorkOrders_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_WorkOrders_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_WorkOrders_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_WorkOrders_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
