...
module Manager =
    ...
    type MaterialManager (messageSenders: MessageSenders, eventStore: IEventStore<string>, materialsViewer: AggregateViewer<Material>, productsViewer: AggregateViewer<Product>, workOrdersViewer: AggregateViewer<WorkOrder>) =
        new() =
            let materialsViewer: AggregateViewer<Material> = getAggregateStorageFreshStateViewer<Material, MaterialEvents, string> pgEventStore
            let productsViewer: AggregateViewer<Product> =  getAggregateStorageFreshStateViewer<Product, ProductEvents, string> pgEventStore
            let workOrdersViewer: AggregateViewer<WorkOrder> =  getAggregateStorageFreshStateViewer<WorkOrder, WorkOrderEvents, string> pgEventStore
            MaterialManager(NoSender, pgEventStore, materialsViewer, productsViewer, workOrdersViewer)
        ...
        member this.FailWorkingItem (workOrderId: WorkOrderId) (productId: ProductId) (quantity: Quantity) =
            result
                {
                    let workOrderCommandFail =
                        (WorkOrderCommands.Fail (productId, quantity))
                   
                    return! 
                        workOrderCommandFail
                        |> runAggregateCommand<WorkOrder, WorkOrderEvents, string> workOrderId.Value eventStore messageSenders
                }
