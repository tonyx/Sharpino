...
CREATE TABLE public.aggregate_events_01_User (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_User_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
...
create index ix_01_events_User_id on public.events_01_User(aggregate_id);
create index ix_01_aggregate_events_User_id on public.aggregate_events_01_User(aggregate_id);
create index ix_01_snapshot_User_id on public.snapshots_01_User(aggregate_id);
create index ix_01_snapshot_User_aggregate_id_and_id on public.snapshots_01_User(aggregate_id, id DESC);
create index ix_01_snapshot_User_event_id on public.snapshots_01_User(event_id);
create index ix_01_events_User_timestamp on public.events_01_User("timestamp");
create index ix_01_snapshots_User_timestamp on public.snapshots_01_User("timestamp");
                                                                                                                                                          
CREATE OR REPLACE FUNCTION insert_01_User_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_User_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
...
CREATE OR REPLACE FUNCTION insert_01_User_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_User_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text   
