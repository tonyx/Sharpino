...
CREATE TABLE public.aggregate_events_01_Products (
                                                     id integer DEFAULT nextval('public.aggregate_events_01_Products_id_seq') NOT NULL,
                                                     aggregate_id uuid NOT NULL,
...
create index ix_01_events_Products_id on public.events_01_Products(aggregate_id);
create index ix_01_aggregate_events_Products_id on public.aggregate_events_01_Products(aggregate_id);
create index ix_01_snapshot_Products_id on public.snapshots_01_Products(aggregate_id);
create index ix_01_snapshot_Products_event_id on public.snapshots_01_Products(event_id);
create index ix_01_events_Products_timestamp on public.events_01_Products("timestamp");
create index ix_01_snapshots_Products_timestamp on public.snapshots_01_Products("timestamp");

CREATE OR REPLACE FUNCTION insert_01_Products_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_Products_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_Products_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
