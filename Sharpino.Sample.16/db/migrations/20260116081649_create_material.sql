...
CREATE TABLE public.aggregate_events_01_Materials (
                                                 id integer DEFAULT nextval('public.aggregate_events_01_Materials_id_seq') NOT NULL,
                                                 aggregate_id uuid NOT NULL,
...
create index ix_01_events_Materials_id on public.events_01_Materials(aggregate_id);
create index ix_01_aggregate_events_Materials_id on public.aggregate_events_01_Materials(aggregate_id);
create index ix_01_snapshot_Materials_id on public.snapshots_01_Materials(aggregate_id);
create index ix_01_snapshot_Materials_event_id on public.snapshots_01_Materials(event_id);
create index ix_01_events_Materials_timestamp on public.events_01_Materials("timestamp");
create index ix_01_snapshots_Materials_timestamp on public.snapshots_01_Materials("timestamp");

CREATE OR REPLACE FUNCTION insert_01_Materials_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_Materials_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_Materials_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_Materials_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
