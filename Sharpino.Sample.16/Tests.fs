...
let tests =
    testList "material tests" [
        ...
        testCase "add a material and a product based on that material, then add a workingItem of that single product, retrieve that work order - async"  <| fun _ ->
            setUp ()
            let pistacchio = Material.New "Pistacchio" (Quantity.New 1 |> Result.get)
            let addPistacchio = materialManager.AddMaterial pistacchio
            Expect.isOk addPistacchio "error in adding material"
            ...
            let workOrder =
                ...
                |> Result.get
            let addWorkOrder = 
                materialManager.AddWorkOrderAsync workOrder
                |> Async.AwaitTask
        ...
        testCase "a material is added, a product on that material is added, a work order is added, then the material results consumed - async" <| fun _ ->
            setUp ()
            let pistacchio = Material.New "Pistacchio" (Quantity.New 1 |> Result.get)
            let addPistacchio = materialManager.AddMaterial pistacchio
            Expect.isOk addPistacchio "error in adding material"
        ...
        testCase "add two materials, a product on those materials and then a workOrder, verify both the materials are consumed - async" <| fun _ ->
            setUp ()
            let pistacchio =
                Material.New "Pistacchio" (Quantity.New 1).OkValue
            let cream =
        ...
        testCase "add two materials, a product on those materials and then a workOrder of two items, verify both the materials are consumed accordingly - async" <| fun _ ->
            setUp ()
            let pistacchio =
                Material.New "Pistacchio" (Quantity.New 2).OkValue
            let cream =
        ...
        testCase "when the material is partially insufficient then the work order is not added and any quantity stays the same - async" <| fun _ ->
            let pistacchio =
                Material.New "Pistacchio" (Quantity.New 1).OkValue
            let cream =
                Material.New "Cream" (Quantity.New 2).OkValue
        ...
        testCaseTask "When no workingItem has started yet, then the workOrder state is initialized - async" <| fun _ ->
            task {
                setUp ()
                let pistacchio =
                    Material.New "Pistacchio" (Quantity.New 2).OkValue
        ...
        testCaseTask "when one workingItem has started, then the workOrder state is InProgress - async" <| fun _ ->
            task {
                setUp ()
                let pistacchio =
                    Material.New "Pistacchio" (Quantity.New 2).OkValue
        ...
        testCaseTask "if all workingitem are started and then completed, then the whole order is fullyCompleted - async" <| fun _ ->
            task {
                setUp ()
                let pistacchio =
                    Material.New "Pistacchio" (Quantity.New 2).OkValue
        ...
        testCaseTask "a workOrder with a single workingitem starts and then it fails, the related material quantities are restored - async" <| fun _ ->
            task {
                setUp ()
                let pistacchio =
                    Material.New "Pistacchio" (Quantity.New 1).OkValue
