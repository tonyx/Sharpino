...
CREATE TABLE public.aggregate_events_01_reservation (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_reservation_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
                                                    event_id integer
);
...
CREATE OR REPLACE FUNCTION insert_01_reservation_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_reservation_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
inserted_id integer;
BEGIN
INSERT INTO events_01_reservation(event, aggregate_id, timestamp, md)
VALUES(event_in::text, aggregate_id, now(), md) RETURNING id INTO inserted_id;
return inserted_id;
END;
$$;

CREATE OR REPLACE FUNCTION insert_01_reservation_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
inserted_id integer;
    event_id integer;
BEGIN
    event_id := insert_01_reservation_event_and_return_id(event_in, aggregate_id);
...
CREATE OR REPLACE FUNCTION insert_md_01_reservation_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
...
inserted_id integer;
    event_id integer;
BEGIN
    event_id := insert_md_01_reservation_event_and_return_id(event_in, aggregate_id, md);
