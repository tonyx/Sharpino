...
module PgStorage =
    ...
    type PgStorage(connection: string, serializer: ISerializer) =
        member inline this.ConvertToJson<'A when 'A: (member Serialize: ISerializer -> Json )> (snapshot: 'A) =
            snapshot.Serialize
    ...
    type PgStorageRef(connection: string) =
        interface IStorageRef with
            ...
            member this.SetSnapshot version (id: int, snapshot: Json) name =
                ...
                let command = sprintf "INSERT INTO snapshots%s%s (event_id, snapshot, timestamp) VALUES (@event_id, @snapshot, @timestamp)" version name
                // let serializedSnapshot = serializer.Serialize<'A> snapshot
                let tryEvent = ((this :> IStorageRef).TryGetEvent version id name)
                match tryEvent with
                | None -> Error (sprintf "event %d not found" id)
                | Some event -> 
                    let _ =
