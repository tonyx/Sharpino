    ...
    type PgEventStore(connection: string, readAsText: RowReader -> (string -> string)) =
            ...
            task {
                ...
                try
                    ...
                    for event in events do
                        ...
                        results.Add(ids)
                        // advance so the next event checks against the one just written
                        currentEventId <- ids
        ...
        interface IEventStore<string> with
            ...
            member this.MultiAddAggregateEventsMdAsync2
                (arg: List<EventId * List<Json> * Version * Name * AggregateId>, md: Metadata, extraConstraints: Map<AggregateId * string, EventId>, ?ct: CancellationToken)
                =
                ...
                task {
                    ...
                    try
                        ...
                        let result =
                                    ...
                                    |>> fun (eventId, events, version, name, aggregateId) ->
                                        ...
                                        for event in events do
                                            let command =
                                                ...
                                                cmd.Parameters.AddWithValue("@extra_aggregate_ids", extraStreamsAggregateIds) |> ignore
                            ...
                            with _ as ex ->
                                logger.LogError(
                                    sprintf "MultiAddAggregateEventsMdAsync2. An error occurred: %A" ex.Message
                                )
                                transaction.Rollback()
                                ex.Message |> Error
                        ...
                        try
                            ...
                            conn.Close()
                    with _ as ex ->
                        logger.LogError(sprintf "MultiAddAggregateEventsMdAsync2. An error occurred: %A" ex.Message)
                        return (Error ex.Message)
                }
            ...
            member this.AddAggregateEventsMdAsync2
                ...
                task {
                    logger.LogDebug(
                        sprintf "AddAggregateEventsMdAsync2 %s %s %A %A %s" version name aggregateId events md
                    )
            ...
            member this.SetInitialAggregateStateAndAddEventsMd
                ...
                events
                =
                logger.LogDebug "SetInitialAggregateStateAndAddEvents"
            ...
            member this.SetInitialAggregateStateAndAddAggregateEventsMd
                ...
                events
                =
                logger.LogDebug "SetInitialAggregateStateAndAddAggregateEventsMd"
            ...
            member this.SetInitialAggregateStateAndMultiAddAggregateEventsMd
                ...
                events
                =
                logger.LogDebug "SetInitialAggregateStateAndMultiAddAggregateEvents"
