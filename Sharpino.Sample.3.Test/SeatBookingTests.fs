...
let aggregateRowRefactoredTests =
    ...
    testList "pgEventStore seatBookingTests" [
        ...
        testCase "add a row reference and then seat to it. retrieve the seat then - Ok" <| fun _ ->
            setUp()
            let eventStore = PgEventStore connection
        ...
        testCase "add a row reference and then some seats to it. Retrieve the seats then - Ok" <| fun _ ->
            setUp()
            let eventStore = PgEventStore connection
            eventStore.Reset "_01" "_seatrow"
            eventStore.Reset Stadium.Version Stadium.StorageName
        ...
        testCase "add two row references add a row reference and then some seats to it. Retrieve the seats then - Ok" <| fun _ ->
            ...
            let seats2 = [ { Id = 6; State = Free; RowId = None }
        ...
        testCase "book free seats among two rows - Ok" <| fun _ ->
            setUp()
            let eventStore = PgEventStore connection
...
[<Tests>]
let aggregateRowMemoryRefactoredTests =
    ...
    let connection = 
        ...
        "User Id=safe;"+
        "Password=safe;"
    testList "memory eventStore seatBookingTests" [
        testCase "initially the stadium has no row references - Ok" <| fun _ ->
            setUp()
            let eventStore = MemoryStorage()
            eventStore.Reset "_01" "_seatrow"
